...
if (isset($_GET['showlogin']) && $_GET['showlogin'] === 'True') {
    header("Location: http://unixfr.xyz/studiologin");
    exit();
}
...
<html>
   <head>
      ...
      Roblox.Client.WaitForRoblox(function() {
        RobloxLaunch.StartGame('http://www.unixfr.xyz/game/edit.ashx?PlaceId=' + play_placeId, 'http://www.unixfr.xyz/Login/Negotiate.ashx', 'FETCH', true);
      });
    }
      </script>
   </head>
          ...
          foreach($BasicRows as $GameInfo){
            echo "
              <div class='template' placeid='". $GameInfo['id'] ."'>
               <a href='' class='game-image'><img width='197' height='115' class='' src='https://unixfr.xyz/Thumbs/GameIcon.ashx?assetId=". $GameInfo['id']."&width=576&height=324&version=82' /></a>
               <p>". $GameInfo['name'] ."</p>
              </div>
            ";
          }
