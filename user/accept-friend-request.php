...
function logData($data) {
    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/user/aaaaa2.txt', $data . PHP_EOL, FILE_APPEND);
}
...
$recipient = $_POST['requesterUserId'] ?? null;
if (!$recipient) {
...
// New function to accept a friend request
function acceptFriendRequest($uID, $recipient) {
    include_once($_SERVER['DOCUMENT_ROOT'] . '/config.php');
    global $MainDB;

    logData("Attempting to accept friend request from $recipient to $uID");

    if (userExists($uID) && userExists($recipient)) {
        logData("Both users exist");

        $friendRequest = friendRequestExists($recipient, $uID);
        ...
        if ($friendRequest) {
            ...
            logData("Inserting new friend relationship");
            $newFriend = $MainDB->prepare("INSERT INTO friends (user1, user2) VALUES (:u1, :u2)");
            $newFriend->bindParam(":u1", $uID, PDO::PARAM_INT);
            $newFriend->bindParam(":u2", $recipient, PDO::PARAM_INT);
            $newFriend->execute();
            ...
            logData("Updating friends count for user2");
            $updateUser2FriendsCount = $MainDB->prepare("UPDATE users SET friends = friends + 1 WHERE id = :u");
            $updateUser2FriendsCount->bindParam(":u", $recipient, PDO::PARAM_INT);
            $updateUser2FriendsCount->execute();
            ...
            $response = json_encode(["message" => "The friend request does not exist"]);
            logData("Output: " . $response);
            die($response);
        }
    } else {
        ...
        if (!userExists($recipient)) {
            logData("User with ID $recipient does not exist");
        }
        $response = json_encode(["message" => "One or both users do not exist"]);
        logData("Output: " . $response);
        die($response);
...
acceptFriendRequest($uID, $recipient);
?>
